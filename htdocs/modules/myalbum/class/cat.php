...
class MyalbumCat extends XoopsObject
{
    ...
    function getURL($uid, $num, $pos, $view) {
    	...
    	if ($GLOBALS['myalbumModuleConfig']['htaccess']) {
    		return XOOPS_URL.'/'.$GLOBALS['myalbumModuleConfig']['baseurl'].'/'.str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $this->getVar('title')).'/cat,'.$this->getVar('cid').','.$uid.','.$num.','.$pos.','.$view.$GLOBALS['myalbumModuleConfig']['endofurl'];
    	} else {
