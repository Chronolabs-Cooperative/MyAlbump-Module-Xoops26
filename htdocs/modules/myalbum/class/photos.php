...
class MyalbumPhotos extends XoopsObject
{
    ...
    function getURL() {
		...
		if ($GLOBALS['myalbumModuleConfig']['htaccess']) {
			...
			$url = XOOPS_URL.'/'.$GLOBALS['myalbumModuleConfig']['baseurl'].'/'.str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $cat->getVar('title')).'/'.str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $this->getVar('title')).'/'.$this->getVar('lid').','.$this->getVar('cid').$GLOBALS['myalbumModuleConfig']['endofurl'];
		} else {
	...
	function getEditURL() {
		...
		if ($GLOBALS['myalbumModuleConfig']['htaccess']) {
			$cat_handler = $GLOBALS['xoops']->getModuleHandler('cat', $GLOBALS['mydirname']);
			$cat = $cat_handler->get($this->getVar('cid'));
			$url = XOOPS_URL.'/'.$GLOBALS['myalbumModuleConfig']['baseurl'].'/'.str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $cat->getVar('title')).'/'.str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $this->getVar('title')).'/edit,'.$this->getVar('lid').','.$this->getVar('cid').$GLOBALS['myalbumModuleConfig']['endofurl'];
		} else {
	...
	function getRateURL() {
		...
		if ($GLOBALS['myalbumModuleConfig']['htaccess']) {
			$cat_handler = $GLOBALS['xoops']->getModuleHandler('cat', $GLOBALS['mydirname']);
			$cat = $cat_handler->get($this->getVar('cid'));
			$url = XOOPS_URL.'/'.$GLOBALS['myalbumModuleConfig']['baseurl'].'/' . str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $cat->getVar('title')) . '/' . str_replace(array('_', ' ', ')', '(', '&', '#'), '-', $this->getVar('title')) . '/rate,' . $this->getVar('lid') . ',' . $this->getVar('cid') . $GLOBALS['myalbumModuleConfig']['endofurl'];
		} else {
    ...
    function toArray($justVar = false) {
    	...
    	$catHandler = $GLOBALS['xoops']->getModuleHandler('cat');
		$textHandler = $GLOBALS['xoops']->getModuleHandler('text');
    	$userHandler = $GLOBALS['xoops']->getHandler('user');
    	
    	$ret['photo'] = parent::getValues();
...
class MyalbumPhotosHandler extends XoopsPersistableObjectHandler
{
    ...
    function setStatus($ids, $status = 1) {
    	...
    	switch ($status) {
    		case 1:
    			$catHandler = $GLOBALS['xoops']->getModuleHandler('cat', $this->_dirname);
				...
				foreach($photos as $lid=>$photo) {
					$notification_handler->triggerEvent( 'global' , 0 , 'new_photo' , array( 'PHOTO_TITLE' => $photo->getVar('title') , 'PHOTO_URI' => $photo->getURL() ) ) ;
					if ($photo->getVar('title')>0&&is_object($cats[$photo->getVar('cid')])) {
						$notification_handler->triggerEvent( 'category' , $photo->getVar('cid') , 'new_photo' , array( 'PHOTO_TITLE' => $photo->getVar('title') , 'CATEGORY_TITLE' => $cats[$photo->getVar('cid')]->getVar('title') , 'PHOTO_URI' => $photo->getURL() ) ) ;
					}			
				}
				break;
    	}
    ...
    function delete($photo, $force = true) {
		...
		$votedataHandler = $GLOBALS['xoops']->getModuleHandler('votedata', $this->_dirname);
		$textHandler = $GLOBALS['xoops']->getModuleHandler('text', $this->_dirname);
		$commentsHandler = $GLOBALS['xoops']->getModuleHandler('comments', $this->_dirname);
