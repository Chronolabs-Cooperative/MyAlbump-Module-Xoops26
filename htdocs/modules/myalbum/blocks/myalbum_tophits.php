...
function b_myalbum_tophits_show( $options )
{
	...
	$block = array() ;
	$GLOBALS['myts'] = MyTextSanitizer::getInstance();
	$result = $GLOBALS['xoops']->db->query( "SELECT lid , cid , title , ext , res_x , res_y , submitter , status , date AS unixtime , hits , rating , votes , comments FROM ".$GLOBALS['xoops']->db->prefix( $table_photos )." WHERE status>0 AND $whr_cat ORDER BY hits DESC" , $photos_num , 0 ) ;
