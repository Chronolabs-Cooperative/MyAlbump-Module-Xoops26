...
function b_myalbum_rphoto_show( $options )
{
	...
	if( $cat_limitation ) {
		if( $cat_limit_recursive ) {
			...
			$cattree = new XoopsTree( $GLOBALS['xoops']->dbprefix( $table_cat ) , "cid" , "pid" ) ;
	...
	$GLOBALS['myts'] = MyTextSanitizer::getInstance();
	// Get number of photo 
	$result = $GLOBALS['xoops']->db->query( "SELECT count(lid) FROM ".$GLOBALS['xoops']->db->prefix( $table_photos )." WHERE status>0 AND $whr_cat AND $whr_ext" ) ;
	list( $numrows ) = $GLOBALS['xoops']->db->fetchRow( $result ) ;
	if( $numrows < 1 ) return $block ;

	if( $numrows <= $photos_num ) {
		$result = $GLOBALS['xoops']->db->query( "SELECT lid , cid , title , ext , res_x , res_y , submitter , status , date AS unixtime , hits , rating , votes , comments FROM ".$GLOBALS['xoops']->db->prefix( $table_photos )." WHERE status>0 AND $whr_cat AND $whr_ext" ) ;
	} else {
		$result = $GLOBALS['xoops']->db->query( "SELECT lid FROM ".$GLOBALS['xoops']->db->prefix( $table_photos )." WHERE status>0 AND $whr_cat AND $whr_ext" ) ;
		$lids = array() ;
		$sel_lids = array() ;
		while( list( $lid ) = $GLOBALS['xoops']->db->fetchRow( $result ) ) $lids[] = $lid ;
		...
		$result = $GLOBALS['xoops']->db->query( "SELECT lid , cid , title , ext , res_x , res_y , submitter , status , date AS unixtime , hits , rating , votes , comments FROM ".$GLOBALS['xoops']->db->prefix( $table_photos )." WHERE status>0 AND lid IN ($whr_lid)" ) ;
	}
