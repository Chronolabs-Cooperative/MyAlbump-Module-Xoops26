...
if ( !is_object($GLOBALS['xoops']->user) || !is_object($GLOBALS['xoops']->module) || !$GLOBALS['xoops']->user->isAdmin($GLOBALS['xoops']->module->mid()) ) {
	exit("Access Denied");
}
...
if (isset($previewblock)) {
	...
	$GLOBALS['xoops']->tpl->xoops_setCaching(0);
	if (isset($bid)) {
		$block['bid'] = $bid;
...
//	$GLOBALS['xoops']->redirect("admin.php?fct=blocksadmin",1,_AM_DBUPDATED); GIJ
	$GLOBALS['xoops']->redirect("myblocksadmin.php",1,_AM_DBUPDATED);
	exit();
}
...
if ( $op == "update" ) {
	...
	$GLOBALS['xoops']->redirect('myblocksadmin.php',1,$msg); // GIJ +
}
...
*/
	...
	function myblocksadmin_update_block($bid, $bside, $bweight, $bvisible, $btitle, $bcontent, $bctype, $bcachetime, $bmodule, $options=array())
	{
		...
		$GLOBALS['xoops'] = Xoops::getInstance();
		$xoopsConfig = $GLOBALS['xoops']->getConfigs();
		...
		if ($myblock->store() != false) {
			$sql = sprintf("DELETE FROM %s WHERE block_id = %u", $GLOBALS['xoops']->db->prefix('block_module_link'), $bid);
			$GLOBALS['xoops']->db->query($sql);
			foreach ($bmodule as $bmid) {
				$sql = sprintf("INSERT INTO %s (block_id, module_id) VALUES (%u, %d)", $GLOBALS['xoops']->db->prefix('block_module_link'), $bid, intval($bmid));
				$GLOBALS['xoops']->db->query($sql);
			}
			
			$GLOBALS['xoops']->tpl->xoops_setCaching(2);
			if ($myblock->getVar('template') != '') {
				if ($GLOBALS['xoops']->tpl->is_cached('db:'.$myblock->getVar('template'))) {
					if (!$GLOBALS['xoops']->tpl->clear_cache('db:'.$myblock->getVar('template'))) {
						$msg = 'Unable to clear cache for block ID'.$bid;
					}
				}
			} else {
				if ($GLOBALS['xoops']->tpl->is_cached('block:system|system_dummy.html', 'block'.$bid)) {
					if (!$GLOBALS['xoops']->tpl->clear_cache('block:system|system_dummy.html', 'block'.$bid)) {
						$msg = 'Unable to clear cache for block ID'.$bid;
					}
				}
			}
		...
		// $GLOBALS['xoops']->redirect('admin.php?fct=blocksadmin&amp;t='.time(),1,$msg);
		// exit(); GIJ -
		return $msg ; // GIJ +
	}
