...
{
	if( is_object($criteria) ) $criteria = new CriteriaCompo($criteria) ;
	$criteria->add(new Criteria('`cid`', $cid));
	$photo_handler = $GLOBALS['xoops']->getModuleHandler('photos', $GLOBALS['mydirname']);
...
function myalbum_update_photo( $lid , $cid , $title , $desc , $valid = null , $ext = "" , $x = "" , $y = "" )
{
	...
	if( isset( $valid ) ) {
		...
		if( $valid == 1 ) {
			...
			$notification_handler->triggerEvent( 'category' , $cid , 'new_photo' , array( 'PHOTO_TITLE' => $title , 'CATEGORY_TITLE' => $cat_title , 'PHOTO_URI' => $photo->getURL() ) ) ;
		}
	}
...
{
	if( empty( $table_name_cat ) ) $table_name_cat = $GLOBALS['xoops']->db->prefix($GLOBALS['table_cat']) ;
	if( empty( $table_name_photos ) ) $table_name_photos = $GLOBALS['xoops']->db->prefix($GLOBALS['table_photos']) ;

	$cats[0] = array( 'cid' => 0 , 'pid' => -1 , 'next_key' => -1 , 'depth' => 0 , 'title' => '' , 'num' => 0 ) ;

	$rs = $GLOBALS['xoops']->db->query( "SELECT c.title,c.cid,c.pid,COUNT(p.lid) AS num FROM $table_name_cat c LEFT JOIN $table_name_photos p ON c.cid=p.cid GROUP BY c.cid ORDER BY pid ASC,$order DESC" ) ;

	$key = 1 ;
	while( list( $title , $cid , $pid , $num ) = $GLOBALS['xoops']->db->fetchRow( $rs ) ) {
		$cats[ $key ] = array( 'cid' => intval( $cid ) , 'pid' => intval( $pid ) , 'next_key' => $key + 1 , 'depth' => 0 , 'title' => $GLOBALS['myts']->htmlSpecialChars( $title ) , 'num' => intval( $num ) ) ;
		$key ++ ;
	}
	$sizeofcats = $key ;
