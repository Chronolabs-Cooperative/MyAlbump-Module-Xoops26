...
// for older files
function myalbum_header($render = false)
{
	global $mod_url , $mydirname ;
	$GLOBALS['xoops'] = Xoops::getInstance();
	$GLOBALS['xoops']->tpl->assign( array( 'mod_url' => $mod_url ) ) ;
	if ($render==false)
		$GLOBALS['xoops']->tpl->display( "module:{$mydirname}|{$mydirname}_header.html" ) ;
}


// for older files
function myalbum_footer($render = false)
{
	global $mod_copyright , $mydirname ;
	$GLOBALS['xoops'] = Xoops::getInstance();
	$GLOBALS['xoops']->tpl->assign( array( 'mod_copyright' => $mod_copyright ) ) ;
	if ($render==false)
		$GLOBALS['xoops']->tpl->display( "module:{$mydirname}|{$mydirname}_header.html" ) ;
}
...
function myalbum_get_array_for_photo_assign( $photo , $summary = false )
{
	...
	$comments_handler = $GLOBALS['xoops']->getModuleHandler('comments', $GLOBALS['mydirname']);
	...
	if (is_object($text)) {
		if( $summary ) { $description = extractSummary( $text->getVar('description') ); } else { $description = $text->getVar('description'); }
	} else {
		$description = '';
	}
