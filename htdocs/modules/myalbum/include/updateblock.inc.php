...
if( substr( XOOPS_VERSION , 6 , 3 ) < 2.1 ) {
	...
	$query = "SELECT mid FROM ".$GLOBALS['xoops']->db->prefix('modules')." WHERE dirname='".$modversion['dirname']."' ";
	$result = $GLOBALS['xoops']->db->query($query);
	$record= $GLOBALS['xoops']->db->fetcharray($result);
	if ($record) {
		$mid = $record['mid'];
		$count = count($modversion['blocks']);
		/* $sql = "SELECT * FROM ".$GLOBALS['xoops']->db->prefix('newblocks')." WHERE mid=".$mid." AND block_type ='D'";
		$fresult = $GLOBALS['xoops']->db->query($sql);
		$n_funcnum = $count;
		while ($fblock = $GLOBALS['xoops']->db->fetchArray($fresult)) {
			$bnum = 0;
			for ($i = 1 ; $i <= $count ; $i++) {
			...
			if($bnum) {
				...
				$sql = "UPDATE ".$GLOBALS['xoops']->db->prefix('newblocks')." SET func_num='$n_funcnum' WHERE mid=".$mid." AND bid='".$fblock['bid']."'";
				$iret = $GLOBALS['xoops']->db->query($sql);

			}
		} */
		
		$sql = "SELECT * FROM ".$GLOBALS['xoops']->db->prefix('newblocks')." WHERE mid=".$mid." AND block_type <>'D' AND func_num > $count";
		$fresult = $GLOBALS['xoops']->db->query($sql);
		while ($fblock = $GLOBALS['xoops']->db->fetchArray($fresult)) {
			$local_msgs[] = "Non Defined Block <b>".$fblock['name']."</b> will be deleted";
			$sql = "DELETE FROM ".$GLOBALS['xoops']->db->prefix('newblocks')." WHERE bid='".$fblock['bid']."'";
			$iret = $GLOBALS['xoops']->db->query($sql);
		}
		
		for ($i = 1 ; $i <= $count ; $i++) {
			$sql = "SELECT name,options FROM ".$GLOBALS['xoops']->db->prefix('newblocks')." WHERE mid=".$mid." AND func_num=".$i." AND show_func='".addslashes($modversion['blocks'][$i]['show_func'])."' AND func_file='".addslashes($modversion['blocks'][$i]['file'])."'";
			$fresult = $GLOBALS['xoops']->db->query($sql);
			$fblock = $GLOBALS['xoops']->db->fetchArray($fresult);
