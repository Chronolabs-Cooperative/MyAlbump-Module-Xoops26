<?php
	...
	function myalbum_admin_form_admission () {
		...
		return getFormHtml($form);
	}
	
	function myalbum_admin_form_export() {
		$irs = $GLOBALS['xoops']->db->query( "SELECT c.imgcat_id,c.imgcat_name,c.imgcat_storetype,COUNT(i.image_id) AS imgcat_sum FROM ".$GLOBALS['xoops']->db->prefix("imagecategory")." c NATURAL LEFT JOIN ".$GLOBALS['xoops']->db->prefix("image")." i GROUP BY c.imgcat_id ORDER BY c.imgcat_weight" ) ;
		$imgcat_options = '' ;
		while( list( $imgcat_id,$imgcat_name,$imgcat_storetype,$imgcat_sum ) = $GLOBALS['xoops']->db->fetchRow( $irs ) ) {
...
<input type='submit' name='imagemanager_export' value='"._GO."' onclick='return confirm(\""._AM_MB_EXPORTCONFIRM."\");' />
	...
	function myalbum_admin_form_import_myalbum(){
		...
		$mrs = $GLOBALS['xoops']->db->query( "SELECT dirname FROM ".$GLOBALS['xoops']->db->prefix("modules")." WHERE dirname like 'myalbum%'" ) ;
		$frm = '';
		while( list( $src_dirname ) = $GLOBALS['xoops']->db->fetchRow( $mrs ) ) {
			if( $GLOBALS['mydirname'] == $src_dirname ) continue ;
			
			$module =& $module_handler->getByDirname( $src_dirname ) ;
			if( ! is_object( $module ) ) continue ;
			
			if( ! $GLOBALS['xoops']->user->isAdmin( $module->getVar('mid') ) ) continue ;
			
			$myalbum_cat_options = myalbum_get_cat_options( 'title' , 0 , '--' , '----' , $GLOBALS['xoops']->db->prefix( "{$src_dirname}_cat" ) , $GLOBALS['xoops']->db->prefix( "{$src_dirname}_photos" ) ) ;
	...
	function myalbum_admin_form_import_imagemanager() {
		...
		$frm = '';
		if( $sysperm_handler->checkRight('system_admin', XOOPS_SYSTEM_IMAGE, $GLOBALS['xoops']->user->getGroups() ) ) {
			// only when user has admin right of system 'imagemanager'
			$irs = $GLOBALS['xoops']->db->query( "SELECT c.imgcat_id,c.imgcat_name,COUNT(i.image_id) AS imgcat_sum FROM ".$GLOBALS['xoops']->db->prefix("imagecategory")." c NATURAL LEFT JOIN ".$GLOBALS['xoops']->db->prefix("image")." i GROUP BY c.imgcat_id ORDER BY c.imgcat_weight" ) ;
			$imgcat_options = '' ;
			while( list( $imgcat_id,$imgcat_name,$imgcat_sum ) = $GLOBALS['xoops']->db->fetchRow( $irs ) ) {
	...
	function myalbum_user_form_submit($caller='', $photo, $lid){
		...
		$cat_select = new XoopsFormLabel( _ALBM_PHOTOCAT , $GLOBALS['cattree']->makeSelBox("cid", "title", '--', (isset($_REQUEST['cid'])?intval($_REQUEST['cid']):null), false )) ;
		...
		if ($GLOBALS['myalbumModuleConfig']['tag']) {
			$form->addElement( new XoopsFormTag('tags', 35, 255, $lid) ) ;
		}
		$form->addElement( $cat_select ) ;
