...
$GLOBALS['xoops']->header("{$mydirname}_topten.html");
...
$GLOBALS['xoops']->tpl->assign( 'lang_sortby' , $lang_sortby ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_rank' , _ALBM_RANK ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_title' , _ALBM_TITLE ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_category' , _ALBM_CATEGORY ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_hits' , _ALBM_HITS ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_rating' , _ALBM_RATING ) ;
$GLOBALS['xoops']->tpl->assign( 'lang_vote' , _ALBM_VOTE ) ;
...
foreach( $cat_handler->getObjects( $criteria, true ) as $cid => $cat ) {
	...
	foreach( $photos_handler->getObjects($criteria, true) as $lid => $photo ) {
		...
		$rankings[$i]['photo'][] = array( 'lid' => $lid , 'cid' => $photo->getVar('cid') , 'rank' => $rank , 'title' => $photo->getVar('title') , 'submitter' => $photo->getVar('submitter') , 'submitter_name' => myalbum_get_name_from_uid( $photo->getVar('submitter') ) , 'category' => $catpath , 'hits' => $photo->getVar('hits') , 'rating' => number_format( $photo->getVar('rating') , 2) , 'votes' => $photo->getVar('votes') ) ;
...
$GLOBALS['xoops']->tpl->assign_by_ref( 'rankings' , $rankings ) ;
