...
if ($modid <= 0 || !is_object($GLOBALS['xoops']->user) || !$GLOBALS['xoops']->user->isAdmin($modid)) {
...
if (!is_object($module) || !$module->getVar('isactive')) {
	$GLOBALS['xoops']->redirect(XOOPS_URL.'/admin.php', 1, _MODULENOEXIST);
	exit();
}
...
if (is_array($_POST['perms']) && !empty($_POST['perms'])) {
	...
	foreach ($_POST['perms'] as $perm_name => $perm_data) {
		foreach( $perm_data['itemname' ] as $item_id => $item_name ) {
			if (false != myDeleteByModule($GLOBALS['xoops']->db,$modid,$perm_name,$item_id)) {
