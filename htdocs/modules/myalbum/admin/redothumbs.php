...
if( $GLOBALS['thumbs_dir'] == $GLOBALS['photos_dir'] ) die( "The directory for thumbnails is same as for photos." ) ;

// check or make thumbs_dir
if( $myalbum_makethumb && ! is_dir( $thumbs_dir ) ) {
	if( $safe_mode_flag ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/admin/",10,"At first create & chmod 777 '$thumbs_dir' by ftp or shell.");
		exit ;
	}
...
if( ! empty( $_POST['submit'] ) ) {
	...
	$result = $GLOBALS['xoops']->db->query( "SELECT lid , ext , res_x , res_y FROM " . $GLOBALS['xoopsDB']->prefix($table_photos) . " ORDER BY lid LIMIT $start , $size") or die( "DB Error" ) ;
	$record_counter = 0 ;
	while( list( $lid , $ext , $w , $h ) = $GLOBALS['xoops']->db->fetchRow( $result ) ) {
		$record_counter ++ ;
		...
		if( $true_w != $w || $true_h != $h ) {
			$GLOBALS['xoops']->db->query( "UPDATE " . $GLOBALS['xoopsDB']->prefix($table_photos) . " SET res_x=$true_w, res_y=$true_h WHERE lid=$lid" ) or die( "DB error: UPDATE photo table." ) ;
...
echo "<h3 style='text-align:left;'>".sprintf(_AM_H3_FMT_RECORDMAINTENANCE,$GLOBALS['xoops']->module->name())."</h3>\n" ;
