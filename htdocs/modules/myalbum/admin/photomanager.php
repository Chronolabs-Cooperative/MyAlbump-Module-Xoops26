...
if( ! empty( $_POST['action'] ) && $_POST['action'] == 'delete' && isset( $_POST['ids'] ) && is_array( $_POST['ids'] ) ) {
	...
	$GLOBALS['xoops']->redirect( "photomanager.php?num=$num&cid=$cid" , 2 , _ALBM_DELETINGPHOTO ) ;
	exit ;
	...
	if( $set ) $GLOBALS['xoops']->db->query( "UPDATE " . $GLOBALS['xoopsDB']->prefix($table_photos) . " SET $set WHERE $whr" ) ;
	if( ! empty( $set_for_text ) ) $GLOBALS['xoops']->db->query( "UPDATE $table_text SET $set_for_text WHERE $whr" ) ;

	$GLOBALS['xoops']->redirect( "photomanager.php?num=$num&cid=$cid" , 2 , _ALBM_DBUPDATED ) ;
	exit ;
...
// Query
$rs = $GLOBALS['xoops']->db->query( "SELECT count(l.lid) FROM " . $GLOBALS['xoopsDB']->prefix($table_photos) . " l LEFT JOIN " . $GLOBALS['xoopsDB']->prefix($table_cat) . " c ON l.cid=c.cid WHERE $whr" ) ;
list( $numrows ) = $GLOBALS['xoops']->db->fetchRow( $rs ) ;
$prs = $GLOBALS['xoops']->db->query( "SELECT l.lid, l.title, l.submitter, l.ext, l.res_x, l.res_y, l.status FROM " . $GLOBALS['xoopsDB']->prefix($table_photos) . " l LEFT JOIN " . $GLOBALS['xoopsDB']->prefix($table_cat) . " c ON l.cid=c.cid WHERE $whr ORDER BY l.lid DESC LIMIT $pos,$num" ) ;
...
$urs = $GLOBALS['xoops']->db->query( "SELECT uid,uname FROM ".$GLOBALS['xoops']->db->prefix("users")." ORDER BY uname" ) ;
while( list( $uid , $uname ) = $GLOBALS['xoops']->db->fetchRow( $urs ) ) {
...
echo "<h3 style='text-align:left;'>".sprintf(_AM_H3_FMT_PHOTOMANAGER,$GLOBALS['xoops']->module->name())."</h3>\n" ;
...
// list part
$col = 0 ;
while( list( $lid , $title , $submitter , $ext , $w , $h , $status ) = $GLOBALS['xoops']->db->fetchRow( $prs ) ) {
