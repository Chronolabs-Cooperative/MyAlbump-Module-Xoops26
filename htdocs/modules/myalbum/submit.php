...
if( ! ( $global_perms & GPERM_INSERTABLE ) ) {
	$GLOBALS['xoops']->redirect( XOOPS_URL."/user.php" , 2 , _ALBM_MUSTREGFIRST ) ;
	exit ;
}
...
if( ! is_dir( $photos_dir ) ) {
	if( $safe_mode_flag ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",10,"At first create & chmod 777 '$photos_dir' by ftp or shell.");
		exit ;
	}

	$rs = mkdir( $photos_dir , 0777 ) ;
	if( ! $rs ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",10,"$photos_dir is not a directory");
		exit ;
...
if( $myalbum_makethumb && ! is_dir( $thumbs_dir ) ) {
	if( $safe_mode_flag ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",10,"At first create & chmod 777 '$thumbs_dir' by ftp or shell.");
		exit ;
	}

	$rs = mkdir( $thumbs_dir , 0777 ) ;
	if( ! $rs ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",10,"$thumbs_dir is not a directory");
		exit ;
...
if( ! is_writable( $photos_dir ) || ! is_readable( $photos_dir ) ) {
	...
	if( ! $rs ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",5,"chmod 0777 into $photos_dir failed");
		exit ;
	}
}
...
if( $myalbum_makethumb && ! is_writable( $thumbs_dir ) ) {
	...
	if( ! $rs ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/$mydirname/",5,"chmod 0777 into $thumbs_dir failed");
		exit ;
	}
}
...
if( ! empty( $_POST['submit'] ) ) {
	...
	if( $cid <= 0 ) {
		$GLOBALS['xoops']->redirect( XOOPS_URL.'/modules/'.$GLOBALS['mydirname']."/submit.php" , 2 , 'Category is not specified.' ) ;
		exit ;
	}
	...
	if( $_FILES[$field]['name'] == '' ) {
		...
		if( $preview_name != '' && is_readable( "$photos_dir/$preview_name" ) ) {
			...
			if( empty( $myalbum_allownoimage ) ) {
				$GLOBALS['xoops']->redirect( 'submit.php' , 2 , _ALBM_NOIMAGESPECIFIED ) ;
				exit ;
			} else {
		...
		$GLOBALS['xoops']->redirect( XOOPS_URL.'/modules/'.$GLOBALS['mydirname']."/submit.php" , 2 , _ALBM_FILEERROR ) ;
		exit ;

	} else {
		if( $GLOBALS['myalbumModuleConfig']['myalbum_canresize'] ) $uploader = new MyXoopsMediaUploader( $GLOBALS['photos_dir'] , explode('|', $GLOBALS['myalbumModuleConfig']['myalbum_allowedmime']) , $GLOBALS['myalbumModuleConfig']['myalbum_fsize'] , null , null , explode('|', $GLOBALS['myalbumModuleConfig']['myalbum_allowedexts']) ) ;
		else $uploader = new MyXoopsMediaUploader( $GLOBALS['photos_dir'] , explode('|', $GLOBALS['myalbumModuleConfig']['myalbum_allowedmime']) , $GLOBALS['myalbumModuleConfig']['myalbum_fsize'] , $GLOBALS['myalbumModuleConfig']['myalbum_width'] , $GLOBALS['myalbumModuleConfig']['myalbum_height'] , explode('|', $GLOBALS['myalbumModuleConfig']['myalbum_allowedexts']) ) ;
	...
	if( ! is_readable( "$photos_dir/$tmp_name" ) ) {
		$GLOBALS['xoops']->redirect( 'submit.php' , 2 , _ALBM_FILEREADERROR ) ;
		exit ;
	}
	...
	if( $status ) {
		...
		if (is_object($cat))
			$notification_handler->triggerEvent( 'category' , $cid , 'new_photo' , array( 'PHOTO_TITLE' => $title , 'CATEGORY_TITLE' => $cat->getVar('title') , 'PHOTO_URI' => $photo_obj->getURL() ) ) ;
	}
	...
	if( $caller == 'imagemanager' ) $redirect_uri = XOOPS_URL."/modules/$mydirname/close.php" ;
	$GLOBALS['xoops']->redirect( $redirect_uri , 2 , _ALBM_RECEIVED ) ;
	exit ;
}
...
if( $caller == 'imagemanager' ) {
	...
	$GLOBALS['xoops']->header('module:'.$GLOBALS["mydirname"].'|'.$GLOBALS["mydirname"].'_submit.html');
	myalbum_header(true) ;
}
...
if( $caller != 'imagemanager' && ! empty( $_POST['preview'] ) ) {
	...
	$GLOBALS['xoops']->tpl->assign( 'photo' , $photo_for_tpl ) ;
	echo "<table class='outer' style='width:100%;'>" ;
	$GLOBALS['xoops']->tpl->display( $GLOBALS['xoops']->path('/modules/'.$GLOBALS["mydirname"].'/templates/'.$GLOBALS["mydirname"]."_photo_in_list.html" )) ;
	echo "</table>\n" ;
...
if( $caller == 'imagemanager' ) {
	...
	$GLOBALS['xoops']->tpl->assign('form', myalbum_user_form_submit($caller, $photo, $lid));
	myalbum_footer(true) ;
